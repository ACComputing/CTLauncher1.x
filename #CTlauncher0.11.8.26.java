/*
 * ╔═══════════════════════════════════════════════════════════════════╗
 * ║  CTLauncher v0.1 — Lightweight Launcher                          ║
 * ║  Bedrock-style dark GUI · 900×560 · Offline Profiles             ║
 * ║  Supports LAN & offline-mode servers (online-mode=false)         ║
 * ║                                                                   ║
 * ║  BUILD:  javac CTLauncher.java                                    ║
 * ║  RUN:    java CTLauncher                                          ║
 * ║  JAR:    jar cfe CTLauncher.jar CTLauncher CTLauncher*.class      ║
...
public class CTLauncher extends JFrame {
    ...
    static final String APP  = "CTLauncher";
    ...
    static void initDirs() {
        ...
        Path home;
        if (os.contains("win"))      home = Paths.get(System.getenv("APPDATA"),".ctlauncher");
        else if (os.contains("mac")) home = Paths.get(System.getProperty("user.home"),
                                              "Library","Application Support","ctlauncher");
        else                         home = Paths.get(System.getProperty("user.home"),".ctlauncher");
        ROOT      = home;
    ...
    JPanel buildSidebar() {
        ...
        // Avatar placeholder — "CT" block
        JPanel avatar = new JPanel() {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setColor(C_GREEN);
                g2.fillRoundRect(0, 0, 36, 36, 6, 6);
                g2.setFont(new Font("SansSerif", Font.BOLD, 14));
                g2.setColor(C_WHITE);
                FontMetrics fm = g2.getFontMetrics();
                g2.drawString("CT", (36 - fm.stringWidth("CT"))/2, (36 + fm.getAscent() - fm.getDescent())/2);
                g2.dispose();
            }
        ...
        sidebarPanel.add(sideNavItem("\uD83C\uDFAE  CTLauncher",   "play"));
        sidebarPanel.add(Box.createVerticalStrut(2));
        ...
        // ── Bottom: Community ──
        sidebarPanel.add(sidebarSep());
        JLabel comm = new JLabel("  Community");
    ...
    JPanel buildPlayPanel() {
        ...
        // Header label
        JLabel hdr = new JLabel("  CTLAUNCHER");
        ...
        JPanel hero = new JPanel() {
            @Override protected void paintComponent(Graphics g) {
                ...
                fm = g2.getFontMetrics();
                String vt = "v" + VER + " \u2022 Offline Profile";
    ...
    JPanel buildSettingsPanel() {
        ...
        JTextArea ta = new JTextArea(
            "How it works:\n\n"
            + "CTLauncher creates offline profiles with a UUID\n"
    ...
    void launchGame(String user, String vid, String ram, String java) throws Exception {
        ...
        if (mca != null) {
            mca = mca.replace("${auth_player_name}", user)
                ...
                .replace("${version_type}", "CTLauncher")
    ...
    long fastDownload(String urlStr, Path dest) throws IOException {
        ...
        c.setRequestProperty("User-Agent","CTLauncher/"+VER);
